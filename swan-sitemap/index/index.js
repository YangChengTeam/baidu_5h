...
Page({
    ...
    onLoad(e) {
        ...
        console.log("onLoad this.data.currentPage ", this.data.currentPage)
    ...
    requestData(currentPage) {
        ...
        swan.request({
            ...
            success: res => {
                ...
                //   console.log("success res.data.code ",res.data.code)
                // if (+res.data.code === 0) {
                // let resData = res.data.data;
                let resData = res.data;
                console.log("resData resData.currentPage ", resData.currentPage)
                // console.log("resData resData.totalPage ", resData.totalPage)
                // console.log("resData resData.list ", resData.list)
                // for (var i = 0; i < resData.list.length; i++) {
                //     let arr = resData.list
                //     arr[i].path = "/" + arr[i].path
                //     console.log("arr[i].path " + i + "  -- ", arr[i].path)
                // }
                // console.log("resData that.data.path ", that.data.path)

                that.totalPage = resData.totalPage
                that.data.currentPage = resData.currentPage
                console.log("resData that.data.currentPage  ----", that.data.currentPage)
                that.setData({
